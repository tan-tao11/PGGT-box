...
class PoseHeadSplit(nn.Module):
    def __init__(self, config):
        ...
        # )
        # 创建独立的旋转和平移分支
        self.rotation_branch = Mlp(
            in_features=self.dim_in,
            hidden_features=self.dim_in // 2,
            out_features=self.rot_dim, # 输出维度为旋转部分
            drop=0,
        )
        self.translation_branch = Mlp(
            in_features=self.dim_in,
            hidden_features=self.dim_in // 2,
            out_features=self.trans_dim, # 输出维度为平移部分
            drop=0,
        )
    ...
    def trunk_fn(self, pose_tokens: torch.Tensor, num_iterations: int) -> list:
        ...
        for _ in range(num_iterations):
            ...
            # --- 修改开始 ---
            # Predict pose delta using separate branches
            pred_rot_delta = self.rotation_branch(trunk_out)
            pred_trans_delta = self.translation_branch(trunk_out)
            
            # 将delta拼接起来
            pred_pose_enc_delta = torch.cat([pred_trans_delta, pred_rot_delta], dim=-1)
            # --- 修改结束 ---
